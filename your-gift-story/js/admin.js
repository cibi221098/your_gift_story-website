...
async function loadOrders() {
  ...
  try {
    ...
    console.log('Orders response status:', res.status, 'data:', data);
    if (!res.ok) {
    ...
    if (_orders.length) {
      ...
      }catch(e){console.log('order_items:',e.message);}
    }
    buildFYOptions();
    renderOrders();
  } catch(e) {
  ...
  document.getElementById('orderDetailBody').innerHTML = `
    <div class="order-detail-section">
      ...
      <div style="background:#f9f7f4;border-radius:12px;padding:1rem;display:grid;grid-template-columns:1fr 1fr;gap:.75rem">
        <div><div style="font-size:11px;color:var(--gray);margin-bottom:3px">Name</div><div style="font-size:14px;font-weight:500">${o.customer_name||'—'}</div></div>
        <div><div style="font-size:11px;color:var(--gray);margin-bottom:3px">Phone</div><div style="font-size:14px;font-weight:500">${o.customer_phone||'—'}</div></div>
        <div style="grid-column:1/-1"><div style="font-size:11px;color:var(--gray);margin-bottom:3px">Delivery Address</div><div style="font-size:13px;font-weight:500">${addressLine}</div></div>
    ...
    <div class="order-detail-section">
      ...
      <div class="order-items-list">${items.map(i => `
        <div class="order-item-row">
          <div><div class="order-item-name">${i.name}</div><div class="order-item-custom">${i.custom||''}</div></div>
    ...
    ${o.notes ? `<div class="order-detail-section"><div class="order-detail-label">Notes</div><div style="background:#f9f7f4;border-radius:10px;padding:.75rem 1rem;font-size:13px">${o.notes}</div></div>` : ''}
    <div class="order-detail-section">
